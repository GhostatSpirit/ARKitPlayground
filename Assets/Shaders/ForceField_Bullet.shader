// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

Shader "Custom/Bullet Force Field"
{
	Properties
	{			
		_FieldTex ("Field Texture" ,2D) = "" {}
		_InnerTint ("Inner Tint", Color) = (1.0, 1.0, 1.0, 1.0)
		_OuterTint ("Outer Tint", Color) = (1.0, 1.0, 1.0, 1.0)		
		_Offset("Mesh Offset", Float) = 0.02	
		_FieldPanSpeedX ("Field Texture Pan Speed X", Float) = 1.0	
		_FieldPanSpeedY ("Field Texture Pan Speed Y", Float) = 1.0
		_InnterOffset("Mask Offset", Float) = 1.0
		_InnerPow("Mask Feather", Float) = 1.0		
		_Static("Static Field", Float) = 1.0

		_EdgeFalloffExp("EdgeFalloff Exp", float) = 0
		_EdgeFalloffPow("EdgeFalloff Power", float) = 0
	}

	Category
	{
		Tags { "Queue"="Transparent" "IgnoreProjector"="True" "RenderType"="Transparent" }
		Blend One One	
		Cull Off Lighting Off ZWrite Off Fog {Mode Off}

		SubShader
		{	
			Pass
			{

				CGPROGRAM
				#pragma vertex vert
				#pragma fragment frag
				#pragma fragmentoption ARB_precision_hint_fastest	
				#pragma target 3.0

				#include "UnityCG.cginc"			
				
				sampler2D _FieldTex;
				fixed4 _FieldTex_ST;
			
				fixed4 _InnerTint;
				fixed4 _OuterTint;
				
				fixed _FieldPanSpeedX;
				fixed _FieldPanSpeedY;
				fixed _Offset;				
				fixed _InnterOffset;			
				fixed _InnerPow;
				fixed _Static;

				fixed _EdgeFalloffExp;
				fixed _EdgeFalloffPow;
			

				struct a2v {					
					fixed4 vertex : POSITION;					
					fixed4 texcoord : TEXCOORD0;
					fixed4 normal : NORMAL;
				};

				struct v2f {
					fixed4 vertex : SV_POSITION;					
					fixed2 uv : TEXCOORD0;					
					fixed4 oPos : TEXCOORD1;
					fixed4 normal: TEXCOORD2;
					fixed4 oPosWorld : TEXCOORD4;
				};

				fixed2 uvPanner(fixed2 uv, fixed x, fixed y)
				{
					fixed t = _Time;
					return fixed2(uv.x + x * t, uv.y + y * t);
				}

				/// VERTEX
				v2f vert (a2v v)
				{
					v2f o;

					v.vertex.xyz += v.normal * _Offset;
					o.vertex = UnityObjectToClipPos(v.vertex);				
					o.oPos = v.vertex;
					o.normal = v.normal;
					o.uv = TRANSFORM_TEX ( v.texcoord, _FieldTex );					
					o.oPosWorld = mul(unity_ObjectToWorld, v.vertex);

					return o;
				}

				/// FRAGMENT
				fixed4 frag (v2f i) : COLOR
				{	
					const int interpolators = 6;
					fixed4 pos[6];				
				
					fixed3 innerMask = 0.0; 
					fixed3 outterMask = 0.0;


					innerMask = saturate(pow(innerMask, _InnerPow));

					fixed field_Tex = tex2D(_FieldTex, uvPanner(i.uv, _FieldPanSpeedX, _FieldPanSpeedY));
					fixed3 fieldColor = lerp(_OuterTint, _InnerTint, field_Tex.r);
					fixed3 maskColor = lerp(_OuterTint, _InnerTint, innerMask);
					fixed3 final = fieldColor * field_Tex * innerMask + innerMask * maskColor + fieldColor * _Static * field_Tex;
					
					float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.oPosWorld.xyz);

					float viewDot = abs(dot(viewDirection, normalize(i.normal)));
					viewDot = pow(viewDot, _EdgeFalloffExp) * _EdgeFalloffPow;
					viewDot = clamp(viewDot, 0, 1);

					return fixed4(final * viewDot, 1.0);
				}

				ENDCG 

			}	// PASS
		}	// SUBSHADER
	} 	// CATEGORY
}	// SHADER
